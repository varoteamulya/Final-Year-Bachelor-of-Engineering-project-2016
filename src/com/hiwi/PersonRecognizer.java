...
public  class PersonRecognizer {
    ...
    public static void saveImageToExternalStorage(Bitmap image,String fileName) {
        String fullPath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/GDVFaces/";
        try
        {
	...
	void add(Mat m, String description) {
		...
		FileOutputStream f;
		try {
			f = new FileOutputStream(mPath+description+"-"+img_count+".jpg",true);
			Log.e("GDVFaces","Saving in app path"+mPath);
			Log.e("GDVFaces", "entered add");
			Log.e("GDVFaces", "count value is "+img_count);
			if(img_count==9){
				Log.e("GDVFaces", "Imaged Saved");
				PersonRecognizer.saveImageToExternalStorage(bmp,description);
				img_count=0;
			}
			else
				img_count++;
...
//		IplImage ipl = MatToIplImage(m,-1, -1);
	...
	IplImage BitmapToIplImage(Bitmap bmp, int width, int height) {
		...
		if ((width != -1) || (height != -1)) {
			pu=""+width;
			Log.e("Image Width",pu);
			pu=""+height;
			Log.e("Image Height",pu);
